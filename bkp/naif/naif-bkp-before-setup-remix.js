        ...
        for (let door of doors) {
            if (door instanceof Door || door instanceof MultiDoor) {
                ...
                if (particleThis == true) {
                    content.createParticles(int(random(550, 1500)));
                    content.moreAttractors();
                    content.moreRepulsors();
                    content.updateParticles(int(random(400, 2000)));
    ...
    updateParticles(n) {
        for (let it = 0; it < n; it++) {
            for (let i = this.particles.length - 1; i >= 0; i--) {
                ...
                // p.checkProximity(this.repulsors, random(120, 160), color(random(col1)), color(random(col2))); //color(0, 255, 0, 50), color(255, 0, 255, 50));  // Vert à Magenta
...
class Particle {
    constructor(x, y) {
        ...
        this.lifespan = 350;
        this.col = 255; //color(random(col2)); 
    ...
    show(g) {
        let strokeSize = map(this.lifespan, 0, 350, 0, 4);
        push();
        g.noFill();
