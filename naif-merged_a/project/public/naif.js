...
function draw() {
    ...
    if (drawBGCubes == true) {
        if (backgroundType === "cubik A") {
          ...
          bgGrad(0, 0, wisW, wisH, c1, c2, backgroundAxis === "X" ? "X" : "Y");
          drawCubesAllB();
        } else if (backgroundType === "cubik alea" && nos === 2) {
          bgGrad(0, 0, wisW, wisH, c1, c2, backgroundAxis === "X" ? "X" : "Y");
          drawCubesRand();
...
function stopping() {
    saveCanvas(`naïf-merged_devel_${$fx.hash}`, 'png');
    noLoop();
}
...
function initStratus() {
  ...
  function drawRise(g, rise) {
    ...
    for (let i = 0; i < numOfLines; i++) {
      ...
      let riseC1 = color(random(colr61));
      riseC1.setAlpha(40);

      g.fill(random(riseC1));
      //g.noFill();
      g.stroke(20); //random(colr12));
